...
void nilPrintUnusedStack(Print* pr) {
  const thread_config_t *tcp = nil_thd_configs;
  //  pr->print(F("Unused Stack: ")); // commented to integrate into metadata lines better--JFA 2015/06/25
  while (tcp->wap) {
    pr->print(fillSize((uint8_t*)tcp->wap, (uint8_t*)tcp->wap + tcp->size));
    pr->print(','); // JFA 2015/06/08: changed from ' ' to be consistent with GEM format 0.8
    tcp++;
  }
